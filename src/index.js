...
const displayController = (() => {
	const createPage = () => {
		...
		let sortableTodos = Sortable.create(workspace, {
			...
			onEnd: function (e) {
				logicController.currentProject.todos.splice(
					e.newDraggableIndex,
					0,
					...logicController.currentProject.todos.splice(e.oldDraggableIndex, 1)
				)
		...
		let sortablProjects = Sortable.create(projectsSort, {
			...
			// might not work in edge cases
			onEnd: function (e) {
				logicController.projectsArray.splice(
					e.newDraggableIndex,
					0,
					...logicController.projectsArray.splice(e.oldDraggableIndex, 1)
				)
				storage.updateTodoData()
			},
		})
		...
		allTodos.addEventListener('click', (e) => {
			...
			logicController.currentProject = {
				name: '',
				todos: logicController.getAllTodos(),
			}
		...
		byToday.addEventListener('click', (e) => {
			document.querySelector('#searchbar').value = ''
			const forToday = () => {
				logicController.getAllTodos().filter((todo) => todo.dueDate == logicController.getToday())
			}
				...
				logicController
					...
					.filter(
						(todo) =>
							...
							parse(logicController.getToday(), 'dd-MM-yyyy', new Date())
					)
			}
		...
		workspace.addEventListener('newTodo', (e) => {
			logicController.makeTodo(
				e.detail.arguments.name,
				e.detail.arguments.description,
				format(e.detail.arguments.dueDate, 'dd-MM-yyyy'),
				e.detail.arguments.color
			)
		})
		...
		topbar.addEventListener('search', (e) => {
			if (e.detail.value == '')
				return logicController.viewCertainTodos((_) => logicController.currentProject.todos)
			let filteredArray = logicController.currentProject.todos.filter(
				(obj) =>
					obj.title.toLowerCase().indexOf(e.detail.value) >= 0 ||
					obj.description.toLowerCase().indexOf(e.detail.value) >= 0
			)
	...
	const renderProjectsButtons = () => {
		...
		logicController.projectsArray.forEach((project) => {
			...
			const projectButtonWrapper = elFactory('li', {
				class: 'sidebar-selection-wrapper',
			})
			const projectButtonEdit = elFactory(
				'button',
				{ class: 'sidebar-edit edit-btn' },
				`
			...
			const handle = elFactory(
				'div',
				{ class: 'handle sidebar-handle' },
				` 
		...
		projectsSort.children[logicController.projectsArray.indexOf(logicController.currentProject) + 1].classList.add(
			'active'
		)
		storage.updateTodoData()
	}
	...
	const loadData = () => {
		...
		if (data == null || data == undefined) {
			...
			logicController.makeTodo(
				...
				logicController.getToday(),
				'blue'
			)
			logicController.makeTodo(
				'All of your todos are laid out in this section',
				'And they are saved after you close your browser window',
				logicController.getToday(),
				'blue'
			)
			logicController.makeTodo(
				'The sidebar on the left has your projects...',
				'...and a few shortcuts for your convienence too',
				logicController.getToday(),
				'blue'
			)
			storage.updateTodoData()
		...
		for (let i = 0; i < Object.keys(data).length; i++) {
			...
			for (let j = 0; j < Object.keys(data[i].todos).length; j++) {
				const todo = new Todo(
					data[i].todos[j].title,
					data[i].todos[j].description,
					data[i].todos[j].dueDate,
					data[i].todos[j].color
				)
